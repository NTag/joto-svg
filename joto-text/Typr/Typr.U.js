...
Typr.U.glyphToPath = function(font, gid)
{
	...
	if(font.SVG && font.SVG.entries[gid]) {
		...
		let state = {x:0,y:0,stack:[],nStems:0,haveWidth:false,width: font.CFF.Private ? font.CFF.Private.defaultWidthX : 0,open:false};
		let cff=font.CFF, pdct = font.CFF.Private;
		if(cff.ROS) {
			let gi = 0;
...
Typr.U.getPairAdjustment = function(font, g1, g2)
{
	...
	if(font.GPOS) {
		let gpos = font["GPOS"];
		let llist = gpos.lookupList, flist = gpos.featureList;
		let tused = [];
...
Typr.U.stringToGlyphs = function(font, str)
{
	...
	for(let i=0; i<str.length; i++) {
		...
		let cc = str.codePointAt(i); if (cc > 0xffff) i++;
	...
	for(let i=0; i<str.length; i++) {
		...
		if (cc == 2367) { let t=gls[i-1];  gls[i-1]=gls[i];  gls[i]=t;  }
	...
	let gsub = font["GSUB"];  if(gsub==null) return gls;
	let llist = gsub.lookupList, flist = gsub.featureList;

	let cligs = ["rlig", "liga", "mset",  "isol","init","fina","medi",   "half", "pres",
	...
	{
		let fl = flist[fi];  if(cligs.indexOf(fl.tag)==-1) continue;
	...
	for(let j=0; j<tab.tabs.length; j++)
	{
		...
		if(tab.ltype==1) {
			...
			for(let i=0; i<lr.length; i+=2) {
				let cind = lr[i], tab2 = llist[lr[i+1]];
...
Typr.U._glsCovered = function(gls, cvgs, ci) {
	for(let i=0; i<cvgs.length; i++) {
		let ind = Typr._lctf.coverageIndex(cvgs[i], gls[ci+i]);  if(ind==-1) return false;
	}
	return true;
}
...
Typr.U.glyphsToPath = function(font, gls, lineHeight, align, clr)
{
	...
	for(let i=0; i<gls.length; i++)
	{
		let gid = gls[i];  if(gid==-1) continue;
...
Typr.U.pathToSVG = function(path, prec)
{
	...
	for(let i=0; i<path.cmds.length; i++)
	{
		...
		out.push(cmd);
		while(co<cn) {  let c = path.crds[co++];  out.push(parseFloat(c.toFixed(prec))+(co==cn?"":" "));  }
	}
	return out.join("");
}
